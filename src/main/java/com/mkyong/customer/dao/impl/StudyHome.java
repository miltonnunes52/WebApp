package com.mkyong.customer.dao.impl;

// default package
// Generated 10/Out/2014 16:22:13 by Hibernate Tools 3.6.0

import static org.hibernate.criterion.Example.create;

import java.util.List;

import javax.naming.InitialContext;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.hibernate.LockMode;
import org.hibernate.SessionFactory;

import com.mkyong.customer.dao.StudyHomeInt;
import com.mkyong.customer.model.Study;

/**
 * Home object for domain model class Study.
 * 
 * @see .Study
 * @author Hibernate Tools
 */
public class StudyHome implements StudyHomeInt {

	private static final Log log = LogFactory.getLog(StudyHome.class);

	private final SessionFactory sessionFactory = getSessionFactory();

	protected SessionFactory getSessionFactory() {
		try {
			return (SessionFactory) new InitialContext()
					.lookup("SessionFactory");
		} catch (Exception e) {
			log.error("Could not locate SessionFactory in JNDI", e);
			throw new IllegalStateException(
					"Could not locate SessionFactory in JNDI");
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.mkyong.customer.dao.StudyHomeInt#persist(com.mkyong.customer.dao.
	 * Study)
	 */
	@Override
	public void persist(Study transientInstance) {
		log.debug("persisting Study instance");
		try {
			sessionFactory.getCurrentSession().persist(transientInstance);
			log.debug("persist successful");
		} catch (RuntimeException re) {
			log.error("persist failed", re);
			throw re;
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.mkyong.customer.dao.StudyHomeInt#attachDirty(com.mkyong.customer.
	 * dao.Study)
	 */
	@Override
	public void attachDirty(Study instance) {
		log.debug("attaching dirty Study instance");
		try {
			sessionFactory.getCurrentSession().saveOrUpdate(instance);
			log.debug("attach successful");
		} catch (RuntimeException re) {
			log.error("attach failed", re);
			throw re;
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.mkyong.customer.dao.StudyHomeInt#attachClean(com.mkyong.customer.
	 * dao.Study)
	 */
	@Override
	public void attachClean(Study instance) {
		log.debug("attaching clean Study instance");
		try {
			sessionFactory.getCurrentSession().lock(instance, LockMode.NONE);
			log.debug("attach successful");
		} catch (RuntimeException re) {
			log.error("attach failed", re);
			throw re;
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.mkyong.customer.dao.StudyHomeInt#delete(com.mkyong.customer.dao.Study
	 * )
	 */
	@Override
	public void delete(Study persistentInstance) {
		log.debug("deleting Study instance");
		try {
			sessionFactory.getCurrentSession().delete(persistentInstance);
			log.debug("delete successful");
		} catch (RuntimeException re) {
			log.error("delete failed", re);
			throw re;
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.mkyong.customer.dao.StudyHomeInt#merge(com.mkyong.customer.dao.Study)
	 */
	@Override
	public Study merge(Study detachedInstance) {
		log.debug("merging Study instance");
		try {
			Study result = (Study) sessionFactory.getCurrentSession().merge(
					detachedInstance);
			log.debug("merge successful");
			return result;
		} catch (RuntimeException re) {
			log.error("merge failed", re);
			throw re;
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see com.mkyong.customer.dao.StudyHomeInt#findById(java.lang.Integer)
	 */
	@Override
	public Study findById(java.lang.Integer id) {
		log.debug("getting Study instance with id: " + id);
		try {
			Study instance = (Study) sessionFactory.getCurrentSession().get(
					"Study", id);
			if (instance == null) {
				log.debug("get successful, no instance found");
			} else {
				log.debug("get successful, instance found");
			}
			return instance;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
		}
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * com.mkyong.customer.dao.StudyHomeInt#findByExample(com.mkyong.customer
	 * .dao.Study)
	 */
	@Override
	public List<Study> findByExample(Study instance) {
		log.debug("finding Study instance by example");
		try {
			List<Study> results = (List<Study>) sessionFactory
					.getCurrentSession().createCriteria("Study")
					.add(create(instance)).list();
			log.debug("find by example successful, result size: "
					+ results.size());
			return results;
		} catch (RuntimeException re) {
			log.error("find by example failed", re);
			throw re;
		}
	}
}
